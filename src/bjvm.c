...
DECLARE_ASYNC(bjvm_interpreter_result_t, init_cached_classdescs,
              bjvm_initialize_class_t ic;
              bjvm_classdesc **cached_classdescs;
              , bjvm_thread *thread);

DEFINE_ASYNC(bjvm_interpreter_result_t, init_cached_classdescs,
             bjvm_thread *thread) {
...
bjvm_interpreter_result_t read_string_to_utf8(bjvm_thread *thread,
                                              heap_string *result,
                                              bjvm_obj_header *obj) {
  short *buf;
  size_t len;
...
bjvm_obj_header *make_string(bjvm_thread *thread, bjvm_utf8 string) {
  bjvm_handle *str = bjvm_make_handle(
      thread, new_object(thread, thread->vm->cached_classdescs->string));
...
bjvm_thread *bjvm_create_thread(bjvm_vm *vm, bjvm_thread_options options) {
  ...
  *VECTOR_PUSH(vm->active_threads, vm->active_thread_count,
  ...
  init_cached_classdescs_t init_cached_classdescs;
  future_t result = ::init_cached_classdescs(&init_cached_classdescs, thr);
  ...
  thr->stack_overflow_error =
      new_object(thr, vm->cached_classdescs->stack_overflow_error);

  bootstrap_lookup_class(thr, STR("java/lang/reflect/Field"));
  bootstrap_lookup_class(thr, STR("java/lang/reflect/Constructor"));

  struct bjvm_native_Thread *java_thr =
      (void *)new_object(thr, vm->cached_classdescs->thread);
  ...
  bjvm_cp_method *make_thread = bjvm_method_lookup(
      vm->cached_classdescs->thread, STR("<init>"),
  ...
  bjvm_cp_method *method =
      bjvm_method_lookup(vm->cached_classdescs->system, STR("initPhase1"),
                         STR("()V"), false, false);
  ...
  method =
      bjvm_method_lookup(vm->cached_classdescs->system,
                         STR("setJavaLangAccess"), STR("()V"), false, false);
...
DEFINE_ASYNC_SL(struct bjvm_native_MethodType *, resolve_mh_mt,
                BJVM_MH_KIND_LAST + 1, bjvm_thread *thread,
  ...
  bjvm_cp_method *make = bjvm_method_lookup(
      thread->vm->cached_classdescs->method_type, STR("makeImpl"),
      STR("(Ljava/lang/Class;[Ljava/lang/Class;Z)Ljava/"
          "lang/invoke/MethodType;"),
      false, false);
  bjvm_stack_value result;
  ...
  bjvm_handle *ptypes_array = bjvm_make_handle(
      thread, CreateObjectArray1D(thread, thread->vm->cached_classdescs->klass,
                                  ptypes_count));
...
DEFINE_ASYNC_SL(struct bjvm_native_MethodHandle *, bjvm_resolve_method_handle,
  ...
  case BJVM_MH_KIND_INVOKE_INTERFACE:
    ...
    AWAIT(bjvm_initialize_class(
        &self->init_class_state, thread,
        info->reference->methodref.class_info->classdesc));
    ...
    bjvm_reflect_initialize_method(thread, method->class_info->classdesc,
                                   self->m);
    ...
    AWAIT(bjvm_initialize_class(&self->init_class_state, thread,
                                self->DirectMethodHandle));
    bjvm_cp_method *make =
    ...
    bjvm_thread_run_leaf(
        thread, make,
        (bjvm_stack_value[]){{.obj = (void *)self->m->reflection_method}},
        &result);
...
void wrap_in_exception_in_initializer_error(bjvm_thread *thread) {
  ...
  int error = bjvm_thread_run_leaf(
...
bjvm_get_constant_pool_mirror(bjvm_thread *thread, bjvm_classdesc *classdesc) {
  ...
  bjvm_classdesc *java_lang_ConstantPool =
      bootstrap_lookup_class(thread, STR("sun/reflect/ConstantPool"));
...
bjvm_interpreter_result_t bjvm_invokevirtual_signature_polymorphic(
  ...
  if (!mts_are_same && !is_invoke_basic) {
    ...
    int status = bjvm_thread_run_root(
        thread, asType,
...
DEFINE_ASYNC(bjvm_value, bjvm_resolve_indy_static_argument, bjvm_thread *thread,
             bjvm_cp_entry *ent, bool *is_object) {
  *is_object = false;
  switch (ent->kind) {
  case BJVM_CP_KIND_INTEGER:
  ...
  case BJVM_CP_KIND_METHOD_TYPE:
    *is_object = true;
    ...
    ASYNC_RETURN((bjvm_value){
        .handle =
            bjvm_make_handle(thread, (void *)ent->method_type.resolved_mt)});
  case BJVM_CP_KIND_METHOD_HANDLE:
    *is_object = true;
    AWAIT(bjvm_resolve_method_handle(&self->resolve, thread,
                                     &ent->method_handle));
    ASYNC_RETURN((bjvm_value){
        .handle = bjvm_make_handle(thread, (void *)self->resolve._result)});
  default: {
    UNREACHABLE();
  }
  }
...
DEFINE_ASYNC(int, indy_resolve, bjvm_thread *thread, bjvm_bytecode_insn *insn,
  ...
  // e.g. LambdaMetafactory.metafactory
  {
    ...
    bjvm_handle *bootstrap_handle =
        bjvm_make_handle(thread, (void *)self->mh._result);
...
#define m (indy->method)
    ...
    bjvm_value handles[256]; // TODO: these are UB in async
    bool is_handle[256];
    self->static_i = 0;
...
DEFINE_ASYNC(bjvm_interpreter_result_t, bjvm_interpret, bjvm_thread *thread,
...
#define RELOAD_STACK_LOCALS()                                                  \
  frame = bjvm_get_plain_frame(*(thread->frames + thread->frames_count - 1));  \
...
#define JMP_INSN continue
    ...
    bjvm_insn_invokedynamic: {
      ...
      AWAIT(indy_resolve(&self->indy_resolve, thread, insn, indy));
    ...
    bjvm_insn_new: {
      ...
      if (self->init_class._result == BJVM_INTERP_RESULT_EXC)
        goto done;
    ...
    bjvm_insn_putstatic: {
      ...
      INTERPRETER_AWAIT(
          bjvm_initialize_class(&self->init_class, thread, class->classdesc));
    ...
    bjvm_insn_invokestatic_resolved: {
      ...
      if (frame->state < INVOKE_STATE_MADE_FRAME) {
        ...
        bjvm_interpret(&invoked_frame->async_frame, thread, invoked_frame,
                       &frame->values[sd - insn->args]);

        status = invoked_frame->async_frame._result;
        if (status != BJVM_INTERP_RESULT_OK) {
          goto done;
        }
      } else {
    ...
    bjvm_insn_invokeitable_monomorphic: {
      ...
      if (likely(frame->state < INVOKE_STATE_MADE_FRAME)) {
        ...
        if (!new_frame)
          goto done;
        frame->state = INVOKE_STATE_MADE_FRAME;
        INTERPRETER_AWAIT(bjvm_interpret(&new_frame->async_frame, thread,
                                         new_frame,
                                         &frame->values[sd - insn->args]));
        status = new_frame->async_frame._result;
...
#define GETSTATIC_IMPL(kind)                                                   \
  ...
  if (raw_frame == final_frame) {
    ...
    ASYNC_RETURN(thread->current_exception != nullptr ? BJVM_INTERP_RESULT_EXC
                                                      : status);
  }
