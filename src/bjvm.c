  ...
  case BJVM_MH_KIND_PUT_STATIC: {
    ...
    bjvm_cp_method *make_member =
            bjvm_method_lookup(self->MemberName, STR("<init>"),
              STR("(Ljava/lang/reflect/Field;)V"), false, false);
    bjvm_stack_value result;
    bjvm_thread_run_leaf(
            thread, make_member,
            (bjvm_stack_value[]){
              {.obj = (void *)member->obj},
              {.obj = (void *)self->f->reflection_field}
            }, nullptr);
...
#define thread (args->thread)
  ...
  case BJVM_MH_KIND_INVOKE_STATIC: {
    ...
    assert(self->method);
    int argc = self->method->descriptor->args_count;
    bjvm_stack_value *invoke_args = args->sp_ - argc;

    invoke_args[0].obj = (void *) mh;
    self->frame = bjvm_push_frame(thread, self->method, invoke_args, argc);
    ...
    if (self->method->descriptor->return_type.base_kind != BJVM_TYPE_KIND_VOID) {
      // Method invocations consume all args
      // and replace with one return value on stack
      invoke_args[0] = self->interpreter_ctx->_result;
    }
...
#define ent args->ent
  ...
  case BJVM_CP_KIND_METHOD_HANDLE:
    ...
    ASYNC_RETURN(
        (bjvm_stack_value){.obj = (void *)get_async_result(bjvm_resolve_method_handle)});
  default: {
    UNREACHABLE();
  }
  }
...
#define m (indy->method)
  ...
  self->invoke_array = bjvm_make_handle(thread,
    CreateObjectArray1D(thread, thread->vm->cached_classdescs->object, m->args_count + 3));
  ...
  for (; self->static_i < m->args_count; ++self->static_i) {
    ...
    ReferenceArrayStore(self->invoke_array->obj, self->static_i + 3, get_async_result(bjvm_resolve_indy_static_argument).obj);
  }
  ...
  bjvm_cp_method *invokeWithArguments = bjvm_method_lookup(self->bootstrap_handle->obj->descriptor, STR("invokeWithArguments"),
                                                   STR("([Ljava/lang/Object;)Ljava/lang/Object;"),
                                                   true, false);

  bjvm_stack_value res;
  bjvm_thread_run_root(thread, invokeWithArguments, (bjvm_stack_value[]){
    { .obj = self->bootstrap_handle->obj },
    { .obj = self->invoke_array->obj }
  }, &res);
...
// todo: these should be async local vars instead of macros for efficiency
#define is_static (frame->method->access_flags & BJVM_ACCESS_STATIC)
#define argc (is_static ? frame->num_locals : frame->num_locals - 1)
#define native_args (bjvm_get_native_args(frame) + (is_static ? 0 : 1))
#define target_handle (is_static ? nullptr : bjvm_get_native_args(frame)[0].handle)
#define native_method_handle ((bjvm_native_callback *) frame->method->native_handle)

  assert(frame && "frame is null");

  if (!native_method_handle->async_ctx_bytes) {
    bjvm_stack_value result =
        ((bjvm_sync_native_callback) native_method_handle->ptr)(thread, target_handle, native_args, argc);
    ASYNC_RETURN(result);
  }

  self->native_struct = malloc(native_method_handle->async_ctx_bytes);
  AWAIT_FUTURE_EXPR(((bjvm_async_native_callback) native_method_handle->ptr)(self->native_struct, thread, target_handle, native_args, argc));
