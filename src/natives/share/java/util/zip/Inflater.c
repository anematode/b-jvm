...
DECLARE_NATIVE("java/util/zip", Inflater, init, "(Z)J") {
  ...
  if (ret != Z_OK) {
    free(stream);
    return (bjvm_stack_value){.l = 0};
  }
  return (bjvm_stack_value){.l = (s64)stream};
}
...
DECLARE_NATIVE("java/util/zip", Inflater, inflateBytesBytes, "(J[BII[BII)J") {
  ...
  if (ret != Z_OK && ret != Z_STREAM_END) {
    int read = src_len - stream->avail_in;
    int written = dst_len - stream->avail_out;
    return (bjvm_stack_value){.l = make_return_value(read, written, true, false)};
  }
  return (bjvm_stack_value){.l = make_return_value(src_len - stream->avail_in, dst_len - stream->avail_out, ret == Z_STREAM_END, false)};
}
...
DECLARE_NATIVE("java/util/zip", CRC32, updateBytes0, "(I[BII)I") {
  ...
  return (bjvm_stack_value){.i = (u32)crc};
}