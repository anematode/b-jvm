...
#define ARGS_VOID                                                                                                      \
  bjvm_thread *thread, bjvm_stack_frame *frame, bjvm_bytecode_insn *insns, int *pc_, int *sp_, int64_t *arg_1,         \
      float *_2, double *_3
#define ARGS_INT                                                                                                       \
  bjvm_thread *thread, bjvm_stack_frame *frame, bjvm_bytecode_insn *insns, int *pc_, int *sp_, int64_t *tos_,          \
      float *_2, double *_3
#define ARGS_DOUBLE                                                                                                    \
  bjvm_thread *thread, bjvm_stack_frame *frame, bjvm_bytecode_insn *insns, int *pc_, int *sp_, int64_t *arg_1,         \
...
DECLARE_ASYNC(int, resolve_getfield_putfield,
  locals(bjvm_cp_field_info *field_info; bjvm_cp_class_info *class),
  arguments(bjvm_thread *thread; bjvm_bytecode_insn *inst; bjvm_stack_frame *frame; bjvm_stack_value *sp_;),
  invoked_methods(invoked_method(bjvm_initialize_class)));
...
static int64_t multianewarray_impl_int(ARGS_INT) {
  DEBUG_CHECK
  SPILL(tos)
  uint16_t temp_sp = sp - bjvm_get_plain_stack(frame);
  if (bjvm_multianewarray(thread, &frame->plain, insn->multianewarray, &temp_sp))
    return 0;

  sp = bjvm_get_plain_stack(frame) + temp_sp;
  NEXT_INT(bjvm_get_plain_stack(frame)[temp_sp - 1].obj)
}
...
static int64_t iinc_impl_void(ARGS_VOID) {
  ...
  DEBUG_CHECK
  int *a = &bjvm_get_plain_locals(frame)[insn->iinc.index].i;
...
bjvm_stack_value bjvm_interpret_2(future_t *fut, bjvm_thread *thread, bjvm_stack_frame *frame_) {
  ...
  if (bjvm_is_frame_native(frame_)) {
    ...
    future_t __native_fut = bjvm_run_native(&ctx);
    assert(__native_fut.status == FUTURE_READY); // todo: fix
  ...
  if (unlikely(frame_->is_async_suspended)) {
    ...
    case CONT_GETSTATIC:
    case CONT_PUTSTATIC:
      __bjvm_fut = resolve_getstatic_putstatic(&cont.ctx.resolve_getstatic_putstatic);

      if (__bjvm_fut.status == FUTURE_NOT_READY) {
    ...
    case CONT_INVOKE:
      result = bjvm_interpret_2(&__bjvm_fut, thread, cont.ctx.interp_call.frame);
      if (__bjvm_fut.status == FUTURE_NOT_READY) {
        *async_stack_push(thread) = cont;
        goto suspend;
      }
    ...
    case CONT_INVOKESIGPOLY:
      __bjvm_fut = bjvm_invokevirtual_signature_polymorphic(&cont.ctx.sigpoly);
  ...
  while (true) {
    frame = bjvm_get_plain_frame_data(frame_);
    sp_ = &frame->stack[stack_depth(frame_)];
